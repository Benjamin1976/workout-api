...
router.put("/exercise/:sessionId/:exerciseId", auth_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let body = Object.assign({}, req.body);
    yield Session_model_1.default.findById(sessionId)
        .then((session) => {
        // if (err) throw new Error(err);
        if (!session)
            return null;
        let exercise = session.exercises[exerciseId];
        //update the proper subdocument
        // let updRecord: ExerciseItemType = ExerciseItemInitialValues;
        //   Object.keys(updRecord).forEach((key) => {
        //     if (body?.[key] !== undefined) {
        //       updRecord[key] = body?.[key];
        //     } else {
        //       delete updRecord[key];
        //     }
        //   });
        // session.exercises[exerciseId] = {...exercise, ...updRecord}
        session.exercises[exerciseId].comments = req.body.comments;
        //then mark it as modified and save it
        session.markModified("brackets.rounds");
        //save the model
        session.save();
        return res.json(session.exercises[exerciseId]);
    })
        .catch((err) => {
        (0, utils_1.log)(1, DBL, lm + `error: ${err.message}}`);
        // res.status(500).json({ msg: "Server Error" });
